...
static void nolock_next_element(Queue *q);
static void nolock_rewind_queue(Queue *q);
static unsigned int nolock_element_in_queue(Queue *q, void *element);
static void nolock_destroy_queue(Queue *q);
static void nolock_add_to_queue(Queue *q, void *element, int priority);
static void *nolock_pointer_to_current(Queue *q);
static int nolock_current_priority(Queue *q);
static unsigned int nolock_end_of_queue(Queue *q);
static void *nolock_nosync_remove_from_front(Queue *q, void *element);
...
static void nolock_add_to_queue(Queue *q, void *element, int priority)
{
      ...
      (q->queue && (q->duplicates || !nolock_element_in_queue(q, element))))
  {
    ...
    else if (q->priority_is_tag_only)
    { // FIFO queue
    ...
    { // priority queue
      ...
      if (!prev)
      { // queue only had one element and new element
        // has higher priority
                             ...
                             FILE *fp)
{
  ...
  nolock_rewind_queue(q);
  while (!nolock_end_of_queue(q))
  {
    ret = serialize_element(nolock_pointer_to_current(q), nolock_current_priority(q), fp);
    nolock_next_element(q);
  }
                               ...
                               FILE *fp)
{
  ...
  while ((ret = deserialize_element(element, &priority, fp)) > 0)
  {
    nolock_add_to_queue(q, element, priority);
  }
