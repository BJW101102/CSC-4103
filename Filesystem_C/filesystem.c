...
void fs_print_error(void)
{
    ...
    case FS_OUT_OF_SPACE:
        ...
        printf("FS_OUT_OF_SPACE: Disk is out of space\n");
        break;

    case FS_FILE_NOT_OPEN:
        // Attempted read/write/close/etc. on a file that isn't open
        printf("FS_FILE_NOT_OPEN: File not open\n");
        break;

    case FS_FILE_OPEN:
        // File is already open. Concurrent opens are not supported, and neither is deleting a file that is open.
        printf("FS_FILE_OPEN: File already open\n");
        break;

    case FS_FILE_NOT_FOUND:
        // Attempted open or delete of a file that doesn’t exist
        printf("FS_FILE_NOT_FOUND: File not found\n");
        break;

    case FS_FILE_READ_ONLY:
        // Attempted write to a file opened for READ_ONLY
        printf("FS_FILE_READ_ONLY: File is read-only\n");
        break;

    case FS_FILE_ALREADY_EXISTS:
        // Attempted creation of a file with an existing name
        printf("FS_FILE_ALREADY_EXISTS: File already exists\n");
        break;

    case FS_EXCEEDS_MAX_FILE_SIZE:
        // Seek or write would exceed the maximum file size
        printf("FS_EXCEEDS_MAX_FILE_SIZE: Exceeds max file size\n");
        break;

    case FS_ILLEGAL_FILENAME:
        // Filename begins with a null character
        printf("FS_ILLEGAL_FILENAME: Illegal filename, Filename begins with a null character or Filename exceeds MAX_FILE_NAME_LENGTH\n");
        break;

    case FS_IO_ERROR:
        // FS_IO_ERROR really bad happened
...
{
    fserror = FS_NONE;
    if (strlen(name) > MAX_FILE_NAME_LENGTH || name[0] == '\0'){
...
// Expected Errors: FS_FILE_NOT_FOUND, FS_FILE_OPEN
